...
class Player:
    ...
    def decide(self):
        decision = input('Would you like to "Hold" or "Roll"? ')
...
class Game:
    def __init__(self, player1, player2, die):
        ...
        print(input("Press Enter to flip a coin to determine who will go first."))
...
def newGame():
    new = input('Welcome to Pig, would you like to read the rules, or just start the game? ("Read" / "Start")')
    if new.lower() == "read":
        ...
        new = input('Ready to start? Type "Start" to begin a normal game, or "Timed" to begin a timed game! ')
    if new.lower() == "start":
        player1 = input("Player 1: Human or Computer? ").lower()
        player2 = input("Player 2: Human or Computer? ").lower()
        die = Die()
        if player1 == "human":
            player1 = HumanPlayer()
